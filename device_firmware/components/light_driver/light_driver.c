...
esp_err_t light_driver_init(light_driver_config_t *config)
{
    ...
    // Nếu chưa có dữ liệu lưu trước đó thì khởi tạo mặc định
    g_light_status.mode       = MODE_HSV;
    g_light_status.on         = 1;
    g_light_status.hue        = 120;   // xanh lá
    ...
    // Khởi tạo PWM LED
    iot_led_init(LEDC_TIMER_0, LEDC_LOW_SPEED_MODE,
                 config->freq_hz, config->clk_cfg, config->duty_resolution);
                                 ...
                                 uint16_t *h, uint8_t *s, uint8_t *v)
{
    ...
    double m_max = fmax(red, fmax(green, blue));
double m_min = fmin(red, fmin(green, blue));
...
esp_err_t light_driver_fade_brightness(uint8_t brightness)
{
    ...
    if (g_light_status.mode == MODE_HSV) {
        ...
        if (brightness != 0) {
            ...
            uint8_t max_color = (uint8_t) fmax(fmax(red, green), blue);

            uint8_t change_value    = brightness * 255 / 100 - max_color;
            fade_period_ms = LIGHT_FADE_PERIOD_MAX_MS * change_value / 255;
        } else {
            fade_period_ms = LIGHT_FADE_PERIOD_MAX_MS * (uint32_t)fmax(fmax(red, green), blue) / 255;
