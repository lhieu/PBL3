...
static void wifi_init_sta(void)
{
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "Stored Wi-Fi SSID='%s' (len=%d)",
                 (char *)cfg.sta.ssid, (int)strlen((char *)cfg.sta.ssid));
    } else {
        ESP_LOGW(TAG, "esp_wifi_get_config() failed: %s", esp_err_to_name(err));
    }

    // Optional for debug (ổn định hơn):
    // ESP_ERROR_CHECK(esp_wifi_set_ps(WIFI_PS_NONE));

    ESP_ERROR_CHECK(esp_wifi_start());

    // ❌ BỎ DÒNG NÀY để tránh connect 2 lần:
